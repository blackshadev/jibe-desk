...
final class LinkReversalAction
{
    public static function make(): Action
    {
        return Action::make('linkReversal')
            ...
            ->color('warning')
            ->visible(static fn (BankTransaction $record): bool => $record->status === BankTransactionStatus::Open && !$record->isReversal() && !$record->isReversed())
            ->schema([
                Select::make('reversal_transaction_id')
                    ->label(__('labels.reversal_transaction'))
                    ->options(
                        static fn (BankTransaction $record): array => BankTransaction::query()
                            ->where('id', '!=', $record->id)
