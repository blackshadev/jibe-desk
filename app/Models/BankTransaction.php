...
final class BankTransaction extends Model
{
    ...
    {
        return $this->morphedByMany(Invoice::class, 'reference', 'bank_transaction_references')
            ->withTimestamps();
    }
    ...
    {
        return $this->morphedByMany(PurchaseOrder::class, 'reference', 'bank_transaction_references')
            ->withTimestamps();
    }
    ...
    {
        return DB::table('bank_transaction_links')
            ->where('link_type', 'internal_transfer')
            ->where(fn ($query) => $query
                ->where('bank_transaction_id', $this->id)
    ...
    public function linkedInternalTransfer(): ?self
    {
        ...
        $otherId = (int) $link->bank_transaction_id === $this->id
